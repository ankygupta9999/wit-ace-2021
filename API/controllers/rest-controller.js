...
var discovery;
const version_date = '2018-12-03';

discovery = new DiscoveryV1({
  version: version_date,
  authenticator: new IamAuthenticator({
    apikey: 'ClWjGlWMe_H72Gp-QEVxDE03sglSI_cr-hVlHLbcMqm2',
  }),
  serviceUrl: 'https://api.eu-gb.discovery.watson.cloud.ibm.com/instances/bc3e95fb-11db-48d7-acf9-8bf277290367',
});
/**
 * 
...
function createSupplier(req, res, next) {
    ...
    listsDao.createSupplier(requestBody).then((result) => {
        utils.writeServerJsonResponse(res, result.data, result.statusCode);
    }).catch((err) => {
        next(err);
    });
...
function createBuyer(req, res, next) {
    ...
    listsDao.createBuyer(requestBody).then((result) => {
        utils.writeServerJsonResponse(res, result.data, result.statusCode);
    }).catch((err) => {
        next(err);
    });
...
function createMaterial(req, res, next) {
    ...
    listsDao.createMaterial(requestBody).then((result) => {
        utils.writeServerJsonResponse(res, result.data, result.statusCode);
    }).catch((err) => {
        next(err);
    });
...
function read(req, res, next) {
    ...
    const queryParams = {
        environmentId: 'ffa37d3f-d3f1-4d77-a074-0831c0e73eb5',
        collectionId: 'd049b2a9-3623-4337-9da5-dbed3c5639f6',
        query:'data.raw_material:Plastic'
      };
      
      discovery.query(queryParams)
        .then(queryResponse => {
            utils.writeServerJsonResponse(res, queryResponse.result.results[0].data, queryResponse.status);
...
function watsonSearch(req, res, next) {
    let query = url.parse(req.url, true).query;
    if (query.description) {
        ...
        const queryParams = {
            environmentId: 'ffa37d3f-d3f1-4d77-a074-0831c0e73eb5',
            collectionId: 'd049b2a9-3623-4337-9da5-dbed3c5639f6',
            query:`data.raw_material:${query.description}`
          };
...
module.exports.watsonSearch=watsonSearch;
